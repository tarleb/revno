...
describe('module revno', function ()
  ...
  describe('tostrict', function ()
    ...
    it('returns strings unchanged', function ()
      assert.equals("hallo", revno.tostrict("hallo"))
    end)
    ...
    it('does nothing if the argument is already strict', function ()
      local inline = pandoc.Emph('hi')
      local strict = revno.tostrict(inline)
      local result = revno.tostrict(strict)
      assert.equals(strict, result)
    end)
  end)
end)
